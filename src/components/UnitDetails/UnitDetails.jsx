    ...
    <div className="bg-white border border-grey rounded p-4 mr-2">
      ...
      {unit.occupied ?
        ...
        {unit.leaseFile.length > 0 ?
          <div className="font-bold">Lease File: <a className="hover:text-blue" href={unit.leaseFile} target="_blank " rel="noreferrer">Click to View In New Tab</a></div>
          :
          null
        }
        ...
        <div className="flex flex-col overflow-y-scroll h-48 gap-2 border border-smokeyTopaz rounded p-2">
          {unit.rent.map((u, idx) => (
            <div key={idx} className="flex border border-grey rounded p-1" >
        ...
        <div className="flex flex-col overflow-y-scroll h-40 gap-2 border border-smokeyTopaz rounded p-2">
          {unit.tenants.map((tenant, idx) => (
              <div key={idx} className="flex flex-col border border-grey rounded">
